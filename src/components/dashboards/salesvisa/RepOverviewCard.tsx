...
export default function RepOverviewCard({ stats, onClick }: RepOverviewCardProps) {
    ...
    return (
        <div
            onClick={onClick}
            className="bg-white/[0.03] rounded-xl p-4 border border-white/[0.05] cursor-pointer hover:bg-white/[0.05] transition-all"
        >
            <div className="font-bold mb-3 text-[15px] text-[#E5E7EB]">{stats.name}</div>

            {/* Active (All Time) */}
            <div className="flex justify-between items-end mb-3">
                <div className="text-[26px] text-[#3b82f6] font-bold leading-none">{stats.active}</div>
                <div className="text-right">
                    <div className="text-[11px] text-[#94A3B8] uppercase tracking-wide">Total Active</div>
                    <div className="text-[12px] text-[#94A3B8]">Files Handling</div>
                </div>
            </div>

            {/* Completed (In Period) */}
            <div className="flex justify-between items-end mb-3 pb-3 border-b border-white/[0.1]">
                <div className="text-[26px] text-[#22c55e] font-bold leading-none">{stats.completed}</div>
                <div className="text-right">
                    <div className="text-[11px] text-[#94A3B8] uppercase tracking-wide">Completed</div>
                    <div className={`text-[12px] font-semibold ${colorDiff(diffComp)}`}>
