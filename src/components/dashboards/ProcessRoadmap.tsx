        ...
        <div className="p-8 h-full overflow-y-auto">
            <div className="max-w-7xl mx-auto">
                <div className="text-center mb-16">
                    ...
                    <p className="text-gray-400">A structured timeline to operational clarity.</p>
                </div>
                        ...
                        {[
                            ...
                            <div key={i} className="group relative">
                                <div className="md:flex flex-col items-center text-center">
                                    {/* Icon Circle */}
                                    <div className={`w-24 h-24 rounded-full ${item.bg} ${item.border} border-2 flex items-center justify-center mb-6 relative z-10 group-hover:scale-110 transition-transform duration-300 shadow-[0_0_30px_-5px_rgba(0,0,0,0.5)]`}>
                                        <item.icon className={`w-10 h-10 ${item.color}`} />
                                        <div className="absolute -top-2 -right-2 w-8 h-8 rounded-full bg-gray-800 border border-gray-700 flex items-center justify-center text-xs font-bold text-white">
                                            {item.step}
                                        </div>
                                    </div>

                                    {/* Content Card */}
                                    <div className="bg-white/5 p-6 rounded-2xl border border-white/10 hover:bg-white/10 transition-colors w-full min-h-[280px] flex flex-col backdrop-blur-sm">
                                        <div className="text-xs font-bold tracking-widest text-gray-500 uppercase mb-2">{item.title}</div>
